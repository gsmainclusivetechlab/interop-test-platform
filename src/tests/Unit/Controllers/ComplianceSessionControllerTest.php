...
class ComplianceSessionControllerTest extends TestCase
{
    ...
    public function testComplianceSessionUpdateInVerification()
    {
        ...
        $session = factory(Session::class)->create(['status' => Session::STATUS_IN_VERIFICATION]);

        $response = $this->actingAs($user)
                         ->withSession(['session' => $session])
                         ->put(route('admin.compliance-sessions.update', ['session' =>$session->getKey()]), [
    ...
    {
        $user = factory(User::class)->create(['role' => User::ROLE_ADMIN]);
        $session = factory(Session::class)->create(['status' => Session::STATUS_DECLINED]);
