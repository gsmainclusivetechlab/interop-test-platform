...
class Scenario extends Model
{
     ...
     * @return mixed
     */
    public function scopeWithTestCasesAndTestRunsOfSession(
        $query,
        Session $session,
        array $testRunsId = []
    ) {
        return $query
            ->with([
                'testCases' => function ($query) use ($session, $testRunsId) {
                    $query
                        ->with([
                            'testRuns' => function ($query) use (
                                $session,
                                $testRunsId
                            ) {
                                $query
                                    ->where('session_id', $session->getKey())
                                    ->when($testRunsId, function ($query) use (
                                        $testRunsId
                                    ) {
                        ...
                        ->whereHas('testRuns', function ($query) use (
                            $session,
                            $testRunsId
                        ) {
                            $query
