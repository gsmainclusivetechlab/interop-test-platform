...
class BaseSessionExport implements ExportableReport
{
    ...
    protected function pdfSubHeader(Section $section)
    {
        $type =
            $this->request['type_of_report'] == 'extended'
                ? 'Technical'
                : 'Business';
    ...
    {
        $this->title($section, 'Info');
        $this->line($section, 'Session name', $this->session->name);
        $this->line(
            $section,
            'Session date',
            $this->session->created_at->format('F d, Y')
        );

        $section->addText('&nbsp;');
    }

    protected function pdfBody(Section $section)
    {
        $useCases = UseCase::WithTestCasesAndTestRunsOfSession(
            $this->session,
            $this->request['test_runs']
        )->get();
    ...
    public function exportPdf(Session $session, array $request): PhpWord
    {
        ...
        $this->pdfHeader($section);
        $this->pdfSubHeader($section);
        $this->pdfInfoHeader($section);
        $this->pdfBody($section);
