<div class="centered">
    <table>
        <tr>
            <section class="cards">

                <a class=" btn scenario-card demo1" href="#create-session" v-b-toggle.accordion-1>
                    <h6 id="scenario-heading" class="border-bottom border-primary">
                        <img id="session-icon" src="{{ asset('images/tutorials/Gizmo_RED_Stack.png') }}" class="icon"/>
                        How do I create a new session?
                    </h6>
                    <div id="scenario-desc">
                        Click here to be directed to an interactive demo showing you the steps to create a new session.
                    </div>
                </a><!-- /card-one -->

                <a class=" btn scenario-card demo2" href="#service-provider" v-b-toggle.accordion-2>
                    <h6 id="scenario-heading" class="border-bottom border-primary">
                        <img id="session-icon" src="{{ asset('images/tutorials/Gizmo_RED_Press.png') }}" class="icon"/>
                        How do I execute a session?
                    </h6>
                    <div id="scenario-desc">
                        Click here to learn more about Postman and links to collections you can use to simulate a Service Provider.
                    </div>
                </a><!-- /card-two -->

                <a class=" btn scenario-card" href="#postman-collections" v-b-toggle.accordion-3 >
                    <h6 id="scenario-heading"  class="border-bottom border-primary">
                        <img id="session-icon" src="{{ asset('images/tutorials/logo-postman.png') }}" class="icon" />
                        Postman Collections
                    </h6>
                    <div id="scenario-desc">
                        Click here to be taken to a list of Postman collections you can use on the test platform.
                    </div>
                </a>
            </section>
        </tr>
        <tr>
            <section class="cards">

                <a class=" btn scenario-card" href="#mojaloop-info" v-b-toggle.accordion-4 >
                    <h6 id="scenario-heading"  class="border-bottom border-primary">
                        <img id="session-icon" src="{{ asset('images/tutorials/Gizmo_RED_World.png') }}" class="icon" />
                        Learn more about Mojaloop
                    </h6>
                    <div id="scenario-desc">
                        Click here to learn more about Mojaloop with links to documentations and specifications
                    </div>
                </a>

                <a class=" btn scenario-card" href="#mobile-money-api-info" v-b-toggle.accordion-5 >
                    <h6 id="scenario-heading"  class="border-bottom border-primary">
                        <img id="session-icon" src="{{ asset('images/tutorials/Gizmo_RED_Mobile_Operator.png') }}" class="icon" />
                        Learn more about Mobile Money API
                    </h6>
                    <div id="scenario-desc">
                        Click here to learn more about the Mobile Money API with links to documentations and specifications
                    </div>
                </a>
            </section>
        </tr>
    </table>
</div>
