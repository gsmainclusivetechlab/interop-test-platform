@extends('layouts.sessions.default', $session)
        ...
        <div class="table-responsive mb-0">
            <table class="table table-striped table-hover card-table">
                ...
                @forelse ($testRuns as $testRun)
                    <tr>
                        <td>
                            <a href="{{ route('sessions.test-cases.show', [$testRun->session, $testRun->testCase]) }}">
                                {{ $testRun->testCase->name }}
                            </a>
                        </td>
                        <td>
                            <a href="{{ route('sessions.test-cases.test-runs.show', [$testRun->session, $testRun->testCase, $testRun]) }}">
                                {{ $testRun->uuid }}
                            </a>
                        </td>
                        <td>
