...
@section('content')
    <form class="flow-chart-content" action="{{ route('sessions.register.update', $session) }}" method="POST">
        @csrf
        @method('PATCH')
        <div class="card">
            <div class="row">
                <div class="col-6">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input name="name" value="{{ old('name', $session->name) }}" type="text" class="form-control @error('name') is-invalid @enderror">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="5">{{ old('description', $session->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback">
                                    <strong>{{ $message }}</strong>
                                </span>
                                                            ...
                                                            @foreach($useCase->positiveTestCases as $testCase)
                                                                <li class="list-group-item">
                                                                    <label class="custom-control custom-checkbox mb-0">
                                                                        <input name="test_cases[{{ $testCase->id }}]" value="{{ $testCase->id }}" type="checkbox" class="custom-control-input" {{ old("test_cases.{$testCase->id}", $session->testCases->contains($testCase)) ? 'checked' : '' }}>
